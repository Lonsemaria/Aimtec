...
namespace Krystra_Mid_Series
{
    ...
    public static class Gapcloser
    {
        ...
        public static void Attach(Menu GeneralMenu )
        {
            if (GameObjects.EnemyHeroes.Any(x => x.IsMelee && Gapcloser.Spells.Any(spell => x.ChampionName == spell.ChampionName)))
            {
                var antigap = new Menu("gapcloser", "Anti-Gapcloser");
            {
               antigap.Add(new MenuBool("enabled", "Enable", false));
                    //antigap.Add(antigap);
                if(Constants.MyHero.ChampionName == "Zed")
                    {
                        var Wskill = new Menu("wsk", "W Skill Settings");
                        Wskill.Add(new MenuBool("enabled", "Use W"));
                        Wskill.Add(new MenuList("wlogic", "W Skill Logic", new[] { "Use Mouse Position", "Use Ally Turret" }, 1));
                        antigap.Add(Wskill);
                    }
                    antigap.Add(new MenuSeperator(string.Empty));
                    antigap.Add(new MenuSeperator("1","Detected Skills"));
                ...
                {
                    var antigap2 = new Menu(enemy.ChampionName.ToLower(), enemy.ChampionName);
                    {
                        foreach (var spell in Gapcloser.Spells.Where(x => x.ChampionName == enemy.ChampionName))
                        {
                            antigap2.Add(new MenuBool(
                                $"{enemy.ChampionName.ToLower()}.{spell.SpellName.ToLower()}",
                                $"Slot:[ {spell.Slot} ] "));
                        }
                    }
                        antigap.Add(antigap2);
                }
            }
                GeneralMenu.Add(antigap);
        }
                    else
                    {
                GeneralMenu.Add(new MenuSeperator(string.Empty, "Anti-Gapcloser not needed."));
                    }
}
    }
}